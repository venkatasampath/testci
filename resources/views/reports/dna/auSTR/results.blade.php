@if(isset($dnas) && count($dnas) > 0)
    <div class="card-body">
        ...
        <table class="table table-bordered table-striped mav-datatable-report">
            ...
            <th>@lang('labels.austr_sequence_number')</th>
            <th>@lang('labels.austr_sequence_similar')</th>
            <th>@lang('labels.austr_result_status')</th>
            <th>@lang('labels.austr_request_date')</th>
            <th>@lang('labels.austr_recieve_date')</th>
            ...
            @foreach ($dnas as $dna)
                @if(isset($dna) && isset($dna->skeletalelement))
                    <tr>
                        ...
                        @endcan
                        <td class ="table-text"><div><a href={{ url('reports/austrdna/' . $dna->austr_sequence_number)}}>{{$dna->austr_sequence_number}}</a></div></td>
                        <td class="table-text">{{ $dna->austr_sequence_similar }}</td>
                        <td class="table-text">{{ $dna->austr_results_confidence }}</td>
                        <td class="table-text">{{ $dna->austr_request_date }}</td>
                        <td class="table-text">{{ $dna->austr_receive_date }}</td>
