@if(isset($dnas) && count($dnas) > 0)
    <div class="card-body">
        ...
        <table class="table table-bordered table-striped mav-datatable-report">
            ...
            <th>@lang('labels.ystr_sequence_number')</th>
            <th>@lang('labels.ystr_sequence_similar')</th>
            <th>@lang('labels.ystr_result_status')</th>
            <th>@lang('labels.ystr_request_date')</th>
            <th>@lang('labels.ystr_recieve_date')</th>
            ...
            @foreach ($dnas as $dna)
                @if(isset($dna) && isset($dna->skeletalelement))
                    <tr>
                        ...
                        @endcan
                        <td class ="table-text"><div><a href={{ url('reports/ystrdna/' . $dna->ystr_sequence_number)}}>{{$dna->ystr_sequence_number}}</a></div></td>
                        <td class="table-text">{{ $dna->ystr_sequence_similar }}</td>
                        <td class="table-text">{{ $dna->ystr_results_confidence }}</td>
                        <td class="table-text">{{ $dna->ystr_request_date }}</td>
                        <td class="table-text">{{ $dna->ystr_receive_date }}</td>
