    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-striped mav-datatable-report">
                ...
                @foreach ($skeletal_anomaly as $skel_anomaly)
                    <th>{{ $skel_anomaly }}</th>
                @endforeach
                </thead>
                <tbody>
                @foreach ($skeletalelements as $skeletalelement)
                    <tr>
                        ...
                        @foreach($skeletal_anomaly as $skel_anomaly)
                            <td class="table-text text-success">
                                @if($skeletalelement->anomalys->contains('individuating_trait', $skel_anomaly))<div>&#10004</div>@endif
                            </td>
                        @endforeach
                    </tr>
                @endforeach
