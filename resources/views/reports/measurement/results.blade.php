            ...
            <table class="table table-bordered table-striped mav-datatable-report">
                ...
                @foreach ($skeletalelements as $skeletalelement)
                    <tr>
                        ...
                        @foreach( $measurements as $current )
                            @if($skeletalelement->measurements()->where('measurement_id', $current->id)->get()->count() === 1)
                                    <td class="table-text" id="help-slideout" data-m-help_url="{{$current->help_url}}"><div>{{$skeletalelement->getMeasure($current->id)}}</div></td>
                                @else
                                <td class="table-text" id="help-slideout" data-m-help_url="{{$current->help_url}}"><div></div></td>
                            @endif
                        @endforeach
                    </tr>
                @endforeach
