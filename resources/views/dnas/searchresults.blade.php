                ...
                <tr>
                    @can('edit', $dna->skeletalelement)
                        @if ( $open_result_new_tab )
                            <td class="table-text"><div><a href="{{ url('/skeletalelements/'.$dna->se_id) }}" target="_blank">{{ $dna->skeletalelement->key }}</a></div></td>
                        @else
                            <td class="table-text"><div><a href="{{ url('/skeletalelements/'.$dna->se_id) }}">{{ $dna->skeletalelement->key }}</a></div></td>
                        @endif
                    @else
                    ...
                    @can('edit', $dna)
                        @if ( $open_result_new_tab )
                            <td class="table-text"><div><a href="{{ url('/skeletalelements/'.$dna->se_id.'/dnas/'.$dna->id) }}" target="_blank">{{ $dna->sample_number }}</a></div></td>
                        @else
                            <td class="table-text"><div><a href="{{ url('/skeletalelements/'.$dna->se_id.'/dnas/'.$dna->id) }}">{{ $dna->sample_number }}</a></div></td>
                        @endif
                    @else
