...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAeu3jEI7buHLsbMyDq5OZ-Nj5OLlXpCwk" async defer></script>
...
{{--                    @include('layouts.common.header')--}}
        @else
        <coraheader :user="{{$theUser}}" :org="{{$theOrg}}" :project="{{$theUser->getCurrentProject()}}" :role="{{$theUser->role}}"
                    :projects="{{$theUser->projects()->get()}}" :base_url="{{json_encode(url('/'))}}">
        </coraheader>
        @endif

        <!-- Content -->
        <v-main class="p-0" style="min-height: calc(100vh - 64px - 102px)">
