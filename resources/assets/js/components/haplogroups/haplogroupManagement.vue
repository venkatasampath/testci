<template>
    <div class="m-2">
        ...
        <contentheader :trail="this.trail" :title="this.heading" :crud_action="action" model_name="haplogroups" @eventNew="eventNewGo"></contentheader>
        ...
        <haplogroup-crud></haplogroup-crud>
        ...
        <v-data-table :headers="columnVisibility" :items="items" :items-per-page="perPage" :options.sync="options"
                      calculate-widths="" class="elevation-1" :sort-by="['code']"  multi-sort :search="search" :loading="loading"
            ...
            <template v-slot:item.letter="{ item }"><a href="#" @click="editItem(item)"> {{ item.letter }}</a></template>
            <template v-slot:item.subgroup="{ item }">{{ item.subgroup }}</template>
            <template v-slot:item.ancestry="{ item }">{{ item.ancestry }}</template>
...
<script>
    ...
    import haplogroupCrud from "./haplogroupCrud";

    export default {
        name: "haplogroup-management",
        components: {haplogroupCrud},
        props: {
            heading: { type: String, default: "Haplogroup Management" },
        ...
        data() {
            return {
                ...
                trail: [{ text: 'Home', disabled: false, href: '/', },
                    { text: 'Haplogroups', disabled: true, href: '/haplogroups', },
                ],
                headers: [
                    {text: 'Type', align: 'left', value: 'type', width: '6rem', visibility: true},
                    {text: 'Letter', value: 'letter', width: '6rem', visibility: true},
                    {text: 'Subgroup', value: 'subgroup', width: '6rem', visibility: true},
                    {text: 'Ancestry', value: 'ancestry', width: '6rem', visibility: true},
        ...
        created(){
            eventBus.$on('refresh-list', payload => {
                if ((payload.list) && (payload.list === "Haplogroups" || payload.list === "haplogroups")) {
                    this.fetchHaplogroups();
                }
            });
            this.fetchHaplogroups();
        },
        ...
        methods: {
            fetchHaplogroups() {
                this.loading = true;
                axios
                    .request({ url: '/api/haplogroups', method: 'GET',
            ...
            eventNewGo(val) {
                if (val === "haplogroups" || val === "haplogroup") {
                    let payload = { 'title': 'Create New Haplogroup', 'action': 'Create', };
                    eventBus.$emit('show-crud-dialog', payload);
                }
            },
            editItem(item) {
                let payload = { 'title': 'Edit Haplogroup', 'action': 'Edit', 'item': item };
                eventBus.$emit('show-crud-dialog', payload);
            },
            deleteItem(item) {
                let payload = { 'title': 'Delete Haplogroup', 'action': 'Delete', 'item': item };
                eventBus.$emit('show-crud-dialog', payload);
            },
        },
        computed: {
