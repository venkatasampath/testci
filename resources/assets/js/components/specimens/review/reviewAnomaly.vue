        ...
        <v-card>
                ...
                <v-row>
                    <v-col cols="6">
                        <v-autocomplete v-model="state.original" :label="type" filled chips multiple disabled readonly
                                        :items="items" item-text="text" item-value="value" :hint="hint"
                                        persistent-hint>
                        </v-autocomplete>
                    </v-col>
                    <v-col cols="6">
                        <v-autocomplete v-model="state.review" :label="type" filled chips deletable-chips multiple
                                        :items="items" item-text="text" dusk="anomaly-review-option" item-value="value"
                                        :hint="hint"
                                        persistent-hint>
                        </v-autocomplete>
                    </v-col>
            ...
            <v-card-actions>
                <v-row class="justify-center">
                    <v-btn v-if="show_save" color="primary" class="mr-2" @click="save" dusk="anomaly-review-save">
                        <v-icon class="mr-2">mdi-content-save</v-icon>
                        Save
                    </v-btn>
                    <v-btn v-if="show_accept" color="primary" @click="accept" dusk="anomaly-review-accept">
    ...
    export default {
        ...
        data() {
            return {
                ...
                hint: "Please can apply multiple anomalys to this specimen",
                select_all: true,
        ...
        beforeMount() {
            this.items = changeObjectToArray(this.list_anomaly);
        },
        mounted() {
            this.loadReview();
            this.loadOriginal();
        ...
        methods: {
            loadOriginal() {
                ...
                axios.request({
                    ...
                    this.state.original = response.data.data.map(anomaly => anomaly.id.toString());
