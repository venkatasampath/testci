<template>
    <div class="m-2">
        ...
        <contentheader :trail="this.trail" :title="this.heading" :crud_action="action" model_name="instruments" @eventNew="eventNewGo"></contentheader>
        ...
        <instrument-crud></instrument-crud>
        ...
        <v-data-table :headers="columnVisibility" :items="items" :items-per-page="perPage" :options.sync="options"
            ...
            <template v-slot:item.code="{ item }"><a href="#" @click="editItem(item)">{{ item.code }}</a></template>
...
<script>
    ...
    export default {
        ...
        props: {
            heading: { type: String, default: "Instrument Management" },
        ...
        data() {
            return {
                ...
                trail: [{ text: 'Home', disabled: false, href: '/', },
                    { text: 'Instruments', disabled: true, href: '/instruments', },
                ],
                headers: [
                    { text: 'Code', align: 'left', value: 'code', width: '12rem',  visibility: true},
                    { text: 'Module', value: 'module', visibility: true },
                    { text: 'Category', value: 'category', visibility: true},
                    { text: 'Reference', value: 'reference' , visibility: true},
                    { text: 'Active', value: 'active', width: '6rem', visibility: true },
                    ...
                    { text: 'Action', value: 'action', sortable: false, width: '6rem', visibility: true},
                ],
            };
        },
        created(){
            eventBus.$on('refresh-list', payload => {
                if ((payload.list) && (payload.list === "Instruments" || payload.list === "instruments")) {
        ...
        methods: {
            fetchInstruments() {
                this.loading = true;
                axios
                    .request({ url: '/api/instruments', method: 'GET',
            ...
            eventNewGo(val) {
                if (val === "instruments" || val === "instrument") {
                    let payload = { 'title': 'Create New Instrument', 'action': 'Create', };
                    eventBus.$emit('show-crud-dialog', payload);
                }
            },
            editItem(item) {
                let payload = { 'title': 'Edit Instrument', 'action': 'Edit', 'item': item };
                eventBus.$emit('show-crud-dialog', payload);
            },
            deleteItem(item) {
                let payload = { 'title': 'Delete Instrument', 'action': 'Delete', 'item': item };
                eventBus.$emit('show-crud-dialog', payload);
            },
            getBooleanIcon(item) {
                return item === true ? "✔" : "";
