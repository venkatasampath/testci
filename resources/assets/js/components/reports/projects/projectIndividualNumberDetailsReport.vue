<template>
    <div class="m-2">
        <contentheader :trail="this.trail" model_name="project_inumber" :reset_menu="true" @eventReset="reset"
                       ...
                       :info_toolip="true" :info_toolip_text="report_help">
        </contentheader>
        <br>
        <v-card>
            <v-container fluid v-show="showReportCriteria">
                <v-row>
                    ...
                    <v-col cols="12" md="3" sm="12">
                        <v-autocomplete v-model="form.p2" :items="items_provenance2" label="Provenance 2" placeholder="Select Provenance 2"clearable></v-autocomplete>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="12" md="3" sm="12">
                        <v-autocomplete v-model="form.individual_number" :items="items_inumber" :rules="rules.inum" :disabled="disable_inum"
                                        @change="toggleDisable('inumber')" multiple chips deletable-chips label="Individual Number" placeholder="Select Individual Number" clearable></v-autocomplete>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="12" md="3" sm="12">
                        <v-autocomplete v-model="form.sb_id" :items="items_bones" :rules="rules.sb_id" :disabled="disable_boneSide" @change="toggleDisable('bone')"
...
<script>
    ...
    export default {
        name: "projectIndividualNumberDetailsReport",
        props:{
            text: {
                type: Object,
                default: () => {
        ...
        data() {
            return {
                trail: [{ text: 'Home', disabled: false, href: '/'},
                    ...
                    { text:  'Specimens by Individual Number Report', disabled: true, href: '/individualnumberdetails'},
                ],
                ...
                rules: {
                    inum: [ v => !!v || 'Individual is required' ],
        ...
        mounted(){
            if(this.text.individual_number){
                this.form.individual_number = [this.text.individual_number];
                this.generate()
            }
        },
        watch: {
        ...
        methods: {
            ...
            generate(){
                if (!this.disable_generate) {
                    ...
                    axios
                        .request({
                            url: '/api/reports/projects/' + this.project_id + '/individualnumberdetails',
                            method: "GET",
                            headers: {
