        ...
        <v-card>
            ...
            <v-card-text>
                <v-form ref="form" v-model="isFormValid">
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-text-field autocomplete="old-password" v-model="old_password" :name="old_password" label="Old Password" :type="show_old_password?'text':'password'"
                                          :append-icon="show_old_password?'mdi-eye-off':'mdi-eye'" @click:append="() => (show_old_password = !show_old_password)" :rules="rules.passwordRules" required>
                            </v-text-field>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12">
                            <v-text-field autocomplete="new-password" v-model="password" :name="password" label="New Password" :type="show_password?'text':'password'"
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-text-field autocomplete="new-password" v-model="password_confirmation" :name="password_confirmation" label="Confirm New Password" :type="show_password_confirmation?'text':'password'"
                                          :append-icon="show_password_confirmation?'mdi-eye-off':'mdi-eye'" @click:append="() => (show_password_confirmation = !show_password_confirmation)" :rules="rules.passwordRules" required>
                            </v-text-field>
                        </v-col>
                    </v-row>
                </v-form>
...
export default {
    ...
    created() {
        console.log("changePassword component created");
        eventBus.$on('show-change-password-dialog', payload => {
    ...
    methods: {
        close() {
            ...
            this.show_password = this.show_password_confirmation = this.show_old_password = false;
            this.password = this.password_confirmation = this.old_password = "";
            this.index = -1;
        ...
        save () {
            this.loading = true;
            let data = { "password": this.password, "password_confirmation": this.password_confirmation, "old_password": this.old_password ,};
            axios
            .request({ url: '/api/users/' + this.item.id + '/changePassword', method: "POST", data: data,
